...
public class prueba {
    ...
    private static final int POLYNOMIAL = 0b1001; 
    ...
    public static void main(String[] args) {
        ...
        int n = input.length();
        int[] crc3 = calculateCRC3(message, n);
        int[] additionalInfo = new int[CRC_LENGTH];
        for (int i = 0; i < CRC_LENGTH; i++) {
            additionalInfo[i] = crc3[i];
        }
        ...
        System.out.print("Resultado (mensaje + información adicional): ");
        for (int bit : result) {
            System.out.print(bit);
        }
    }

    private static int[] calculateCRC3(int[] message, int n) {
        int[] crc3 = new int[CRC_LENGTH];
        int register = 0x0;
